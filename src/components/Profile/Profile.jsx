...
function Profile() {
  const { user, userEmail, userId, accessToken, profileSrc, username, role } =
    useAuth();
  ...
  const fetchKeys = async () => {
    try {
      const res = await axios.get(`${BACKEND_URL}/display-keys`, {
        headers: {
          Authorization: `Bearer ${accessToken}`,
        },
      });
  ...
  const handleCopyKey = async (keyData) => {
    try {
  ...
  const handleKeyDelete = async (recipient, idx, userId) => {
    try {
      const response = await axios.post(
        `${BACKEND_URL}/api/delete-key`,
        {
          recipient,
          user_id: userId,
        },
    ...
    <div className="Relative flex w-full flex-col sm:flex-row transition-all duration-500 p-2">
      ...
      <div className="basis-[25%] sm:h-[screen] min-w-[14rem] p-2 rounded-md text-center mb-5 ">
        <div className="sticky top-24 flex flex-col  gap-4 text-sm w-full ">
          <div className="basis-1/2 shadow-[0_1px_0_rgba(255,255,255,0.06),0_2px_4px_rgba(0,0,0,0.35)] rounded-md p-2 ">
            ...
            <Link
              to="/Setting"
              className="box-border border-2 border-double flex justify-center items-center gap-3 p-0.5 rounded-md hover:border transition-all hover:scale-105 hover:bg-background/20 active:scale-90  mt-3 mx-2 "
            >
          ...
          <div className="basis-1/2">
            <div className="text-center rounded-lg border shadow-[0_1px_0_rgba(255,255,255,0.06),0_2px_2px_rgba(0,0,0,0.35)] transition-all duration-200 w-full py-2">
              <div className="flex size-16 items-center justify-center rounded-full border border-sky-500/40 bg-sky-500/10  text-sky-400 mx-auto my-4">
                <svg
              ...
              <p className="text-sm text-gray-400 md:px-2">
                Unlimited projects, priority support, and advanced analytics to
      ...
      <div className="basis-[80%] min-h-[85vh] min-w-[14rem] ">
        ...
        <div className="flex flex-grow gap-2 md:flex-row flex-col ">
          ...
          <div
            id="Overall Progress"
            className=" min-h-[16rem] rounded-md basis-[60%] mb-2 shadow-[inset_0_6px_12px_rgba(0,0,0,0.3)]"
          >
        ...
        <div className="border rounded-t-lg">
          ...
          <header className="flex justify-evenly w-full min-w-fit min-h-[2rem] border-b rounded-t-lg px-2 py-2 sm:px-5">

            <div className="flex w-full min-w-fit gap-1 sm:gap-2">
              <button
                onClick={() => handleButtonClick("original")}
                className={`text-sm sm:text-base px-1 py-1 sm:px-2 whitespace-nowrap flex-1 relative outline-none ${
                  activeButton === "uploads" ? "text-primary" : ""
                } `}
              >
                <span>{islargeScreen ? "Original Images" : "Original"}</span>
                <div
                  className={`absolute -bottom-2 left-0 w-full h-0.5 bg-primary transition-opacity duration-300  
                  ${
                    activeButton === "original" ? " opacity-100" : " opacity-0"
                  }`}
                ></div>
              </button>
              ...
              <button
                ...
                className={`text-sm sm:text-base px-1 py-1 sm:px-2 whitespace-nowrap flex-1 relative outline-none ${
                  activeButton === "encrypted" ? "text-primary" : ""
                } `}
              >
                <span>{islargeScreen ? "Encrypted Images" : "Encrypt"}</span>
                <div
                  className={`absolute -bottom-2 left-0 w-full h-0.5 bg-primary transition-opacity duration-300
                  ${
              ...
              <button
                ...
                className={`text-sm sm:text-base px-1 py-1 sm:px-2 whitespace-nowrap flex-1 relative outline-none ${
                  activeButton === "decrypted" ? "text-primary" : ""
                } `}
              >
                <span>{islargeScreen ? "Decrypted Images" : "Decrypt"}</span>
                <div
                  className={`absolute -bottom-2 left-0 w-full h-0.5 bg-primary transition-opacity duration-300  
                  ${
                    activeButton === "decrypted" ? " opacity-100" : " opacity-0"
                  }`}
                ></div>
              </button>
              ...
              <button
                ...
                className={`text-sm sm:text-base px-1 py-1 sm:px-2 whitespace-nowrap flex-1 relative outline-none cursor-pointer ${
                  activeButton === "keys" ? "text-primary" : ""
                } `}
              >
                <span>{"Keys"}</span>
                <div
                  className={`absolute -bottom-2 left-0 w-full h-0.5 bg-primary transition-opacity duration-300  
                  ${activeButton === "keys" ? " opacity-100" : " opacity-0"}`}
                ></div>
              </button>
            </div>
          ...
          <div className="min-h-[13rem]">
            ...
            {activeButton === "keys" ? (
                ...
                {keys.length > 0 ? (
                  <ul className=" list-disc list-inside text-sm flex flex-wrap gap-2 py-2 px-1 justify-center my-2 transition-opacity opacity-0 duration-500 ease-out animate-fadeIn ">
                    {keys.map((key, idx) => (
                      <li
                        key={idx}
                        className="flex flex-col items-center justify-center border border-ring w-48 py-2 rounded bg-card text-card-foreground "
                      >
                        ...
                        <div className="flex gap-2 ">
                          <button
                            onClick={() => handleCopyKey(key.aes_key)}
                            className=" text-xs border border-sky-500 hover:bg-sky-600 text-white px-2 py-1 rounded  active:scale-95 cursor-pointer"
                          >
                            Copy <Copy size={10} className="inline-block" />
                          </button>
                          <span
                            className="text-white border boder-red-400 hover:bg-red-500 rounded px-2 py-1 cursor-pointer active:scale-95 "
                            onClick={() => handleKeyDelete(key.recipient, idx, userId)}
                          >
                            <svg
                              xmlns="http://www.w3.org/2000/svg"
                              width="12"
              ...
              // images
              <div className="">
                <ImageGallery folderName={selectedFolder} userId={userId} />
              </div>
            )}
          </div>
